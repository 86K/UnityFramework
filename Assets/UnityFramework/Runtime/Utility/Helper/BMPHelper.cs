...
namespace UnityFramework.Runtime
{
    ...
    public struct BitmapInfoHeader
    {
        ...
        public int absWidth
        {
            get { return Mathf.Abs(width); }
        }

        public int absHeight
        {
            get { return Mathf.Abs(height); }
        }
    }
    ...
    public class BMPHelper
    {
        ...
        public BMPImage LoadBMP(BinaryReader aReader)
        {
            ...
            if (bmp.info.nBitsPerPixel > 8 && (bmp.info.compressionMethod == BMPComressionMode.BI_BITFIELDS ||
                                               bmp.info.compressionMethod == BMPComressionMode.BI_ALPHABITFIELDS))
            {
        ...
        static void Read32BitImage(BinaryReader aReader, BMPImage bmp)
        {
        ...
        static void Read24BitImage(BinaryReader aReader, BMPImage bmp)
        {
            int w = Mathf.Abs(bmp.info.width);
            int h = Mathf.Abs(bmp.info.height);
        ...
        static void Read16BitImage(BinaryReader aReader, BMPImage bmp)
        {
            int w = Mathf.Abs(bmp.info.width);
            int h = Mathf.Abs(bmp.info.height);
        ...
        static void ReadIndexedImage(BinaryReader aReader, BMPImage bmp)
        {
            int w = Mathf.Abs(bmp.info.width);
            int h = Mathf.Abs(bmp.info.height);
        ...
        static void ReadIndexedImageRLE4(BinaryReader aReader, BMPImage bmp)
        {
            int w = Mathf.Abs(bmp.info.width);
            int h = Mathf.Abs(bmp.info.height);
            Color32[] data = bmp.imageData = new Color32[w * h];
        ...
        static void ReadIndexedImageRLE8(BinaryReader aReader, BMPImage bmp)
        {
            int w = Mathf.Abs(bmp.info.width);
            int h = Mathf.Abs(bmp.info.height);
            Color32[] data = bmp.imageData = new Color32[w * h];
        ...
        static int GetShiftCount(uint mask)
        {
            for (int i = 0; i < 32; i++)
            {
                if ((mask & 0x01) > 0)
        ...
        static uint GetMask(int bitCount)
        {
            uint mask = 0;
        ...
        static bool ReadFileHeader(BinaryReader aReader, ref BMPFileHeader aFileHeader)
        {
        ...
        static bool ReadInfoHeader(BinaryReader aReader, ref BitmapInfoHeader aHeader)
        {
