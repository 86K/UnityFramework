namespace UnityFramework.Editor
{
    ...
    internal class EditorIcons : EditorWindow
    {
        [MenuItem("itsxwz/Editor icons", false, 999)]
        internal static void EditorIconsOpen()
        {
        ...
        static bool m_ViewBigIcons = true;
        Vector2 m_Scroll;
        int m_ButtonSize = 70;
        string m_Search = "";
        ...
        void SearchGUI()
        {
            ...
            {
                if (m_IsWide) GUILayout.Space(10);
        ...
        bool m_IsWide => Screen.width > 550;
        bool m_DoSearch => !string.IsNullOrWhiteSpace(m_Search) && m_Search != "";

        GUIContent GetIcon(string _iconName)
        {
            GUIContent valid = null;
            Debug.unityLogger.logEnabled = false;
            if (!string.IsNullOrEmpty(_iconName)) valid = EditorGUIUtility.IconContent(_iconName);
        ...
        void OnEnable()
        {
            var allIcons = IconNames.Where(x => GetIcon(x) != null);
        ...
        void OnGUI()
        {
            ...
            InitIcons();
            if (!m_IsWide) SearchGUI();

            using (new GUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                GUILayout.Label("Select what icons to show", GUILayout.Width(160));
                m_ViewBigIcons = GUILayout.SelectionGrid(
                    m_ViewBigIcons ? 1 : 0, new string[] {"Small", "Big"},
                    2, EditorStyles.toolbarButton) == 1;

                if (m_IsWide) SearchGUI();
            }

            if (m_IsWide) GUILayout.Space(3);

            using (var scope = new GUILayout.ScrollViewScope(m_Scroll))
            {
                GUILayout.Space(10);

                m_Scroll = scope.scrollPosition;

                m_ButtonSize = m_ViewBigIcons ? 70 : 40;

                // scrollbar_width = ~ 12.5
                var render_width = (Screen.width / ppp - 13f);
                var gridW = Mathf.FloorToInt(render_width / m_ButtonSize);
                var margin_left = (render_width - m_ButtonSize * gridW) / 2;
                ...
                if (m_DoSearch)
                    iconList = m_IconContentListAll.Where(x => x.tooltip.ToLower()
                        .Contains(m_Search.ToLower())).ToList();
                else iconList = m_ViewBigIcons ? m_IconContentListBig : m_IconContentListSmall;
                    ...
                    using (new GUILayout.HorizontalScope())
                    {
                        ...
                        for (var i = 0; i < gridW; ++i)
                        {
                            ...
                            if (GUILayout.Button(icon,
                                m_IconButtonStyle,
                                GUILayout.Width(m_ButtonSize),
                                GUILayout.Height(m_ButtonSize)))
                            {
            ...
            if (m_IconSelected == null) return;

            GUILayout.FlexibleSpace();

            using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(m_ViewBigIcons ? 140 : 120)))
            {
                using (new GUILayout.VerticalScope(GUILayout.Width(130)))
                {
                    GUILayout.Space(2);
                    GUILayout.Button(m_IconSelected, m_IconPreviewStyle, GUILayout.Width(128), GUILayout.Height(m_ViewBigIcons ? 128 : 40));
                    GUILayout.Space(5);
                    GUILayout.FlexibleSpace();
                }
                ...
                {
                    var s = $"Size: {m_IconSelected.image.width}x{m_IconSelected.image.height}";
                    s += "\nIs Pro Skin Icon: " + (m_IconSelected.tooltip.IndexOf("d_") == 0 ? "Yes" : "No");
                    s += $"\nTotal {m_IconContentListAll.Count} icons";
                    GUILayout.Space(5);
                    EditorGUILayout.HelpBox(s, MessageType.None);
                    GUILayout.Space(5);
                    EditorGUILayout.TextField("EditorGUIUtility.IconContent(\"" + m_IconSelected.tooltip + "\")");
                    EditorGUILayout.TextField("EditorGUIUtility.FindTexture(\"" + m_IconSelected.tooltip + "\")");
                    GUILayout.Space(5);

                    if (GUILayout.Button("Export Icon", GUILayout.Height(32)))
                    {
                        var path = EditorUtility.SaveFolderPanel("Save Editor Icon Path", Application.dataPath, "");
                        ...
                        path = Path.Combine(path, $"{m_IconSelected.tooltip}.png");
                        Texture2D icon = m_IconSelected.image as Texture2D;
                        if (icon)
                        {
        ...
        Texture2D _toWritableAndRead(Texture2D _texture2D)
        {
            var renderTexture = new RenderTexture(_texture2D.width, _texture2D.height, 0, RenderTextureFormat.Default, RenderTextureReadWrite.Linear);
            Graphics.Blit(_texture2D, renderTexture);
            Texture2D te = new Texture2D(_texture2D.width, _texture2D.height);
        ...
        static GUIContent m_IconSelected;
        static List<GUIContent> m_IconContentListAll;
        static List<GUIContent> m_IconContentListSmall;
        static List<GUIContent> m_IconContentListBig;
        static GUIStyle m_IconButtonStyle = null;
        static GUIStyle m_IconPreviewStyle = null;

        void AllTheTEXTURES(ref GUIStyle _guiStyle, Texture2D _texture2D)
        {
            _guiStyle.hover.background = _guiStyle.onHover.background = _guiStyle.focused.background = _guiStyle.onFocused.background =
                _guiStyle.active.background = _guiStyle.onActive.background = _guiStyle.normal.background = _guiStyle.onNormal.background = _texture2D;
            _guiStyle.hover.scaledBackgrounds = _guiStyle.onHover.scaledBackgrounds = _guiStyle.focused.scaledBackgrounds =
                _guiStyle.onFocused.scaledBackgrounds = _guiStyle.active.scaledBackgrounds = _guiStyle.onActive.scaledBackgrounds =
                    _guiStyle.normal.scaledBackgrounds = _guiStyle.onNormal.scaledBackgrounds = new Texture2D[] {_texture2D};
        }

        Texture2D Texture2DPixel(Color _color)
        {
            Texture2D t = new Texture2D(1, 1);
            t.SetPixel(0, 0, _color);
            t.Apply();
            return t;
        }
        ...
        void InitIcons()
        {
            if (m_IconContentListSmall != null) return;
            ...
            m_IconPreviewStyle = new GUIStyle(m_IconButtonStyle);
            AllTheTEXTURES(ref m_IconPreviewStyle, Texture2DPixel(new Color(0.15f, 0.15f, 0.15f)));
            
            m_IconContentListSmall = new List<GUIContent>();
            m_IconContentListBig = new List<GUIContent>();
            m_IconContentListAll = new List<GUIContent>();
            ...
            for (var i = 0; i < IconNames.Length; ++i)
            {
                ...
                m_IconContentListAll.Add(ico);

                if (!(ico.image.width <= 36 || ico.image.height <= 36))
                    m_IconContentListBig.Add(ico);
                else m_IconContentListSmall.Add(ico);
            }
        }
